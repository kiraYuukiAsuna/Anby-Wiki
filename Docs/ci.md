...
| Job | 内容 | 本地复现 |
| --- | --- | --- |
| `backend` | `gofmt -l` 非空即失败、`go vet ./...`、`go test ./...`（单元）、对 postgres 服务容器执行迁移后 `TEST_DATABASE_URL=... go test ./... -count=1 -p 1`（PostgreSQL 集成）、`go build ./...`（工作目录 `backend/`，Go 版本取自 `backend/go.mod`） | `make lint-go`、`make test-go`、`make build-go`；集成测试见下节 |
| `web` | `npm ci` 后依次 `npm run typecheck` / `lint` / `test` / `build`（工作目录 `apps/web/`，Node 22） | `make lint-web`（含 typecheck）、`make test-web`、`make build-web` |
...
| `contracts` | 校验 OpenAPI 3.1，并运行 AST / ProposalOperation / Extraction 权威 Schema 与 Go 内嵌副本的字节级防漂移检查 | `make contracts-check`；`cd apps/web && npm ci && npx @openapitools/openapi-generator-cli validate -i ../../contracts/openapi/openapi.yaml`（本地需 JDK） |
...
| `migrations` | 起 `postgres:17` 服务容器（库/用户均为 `wiki`），先校验迁移文件规范，再对空库执行 `go run ./cmd/migrate up` 与 `version` | `make migration-check`；为 `.env` 配置外部 PostgreSQL 后执行 `make migrate-up` |
| `deploy` | 校验 Dockerfile/Compose/shell、备份恢复脚本、迁移目标、失败停止和发布顺序；实构建 `api`/`worker`/`web`/`migrate` 四 target 并检查 non-root/health metadata | `make deploy-check`；Docker 可用时按 `Docs/runbooks/deployment.md` 实构建 |
...
2. `go run ./cmd/migrate up`（`DATABASE_URL` 指向服务容器，`REDIS_URL`/`S3_*`
   ...
   执行 `go test ./... -count=1 -p 1`——发布原子性、不可变触发器、里程碑生命周期等
   PG 集成测试真实跑在 CI 上（核心不变量门禁必须在 CI 常绿，不能接受 skip）。

本地复现：`make pg-start`（免 Docker，端口 55432）后在仓库根执行
`TEST_DATABASE_URL='postgres://wiki@127.0.0.1:55432/wiki?sslmode=disable' make test-go-integration`。
...
### migrations job 的空目录兼容

`cmd/migrate` 只把 `ErrNoChange` 视为成功，但迁移目录为空（或仅 README.md）时，
golang-migrate 返回 source 层的 `fs.ErrNotExist`，不是 `ErrNoChange`。因此 CI
按 `migrations/*.up.sql` 是否存在做条件执行；当前仓库已有初始化迁移，会真实执行
`up` 与 `version`，条件分支仅保留对仓库初始化阶段的兼容。

另外 `cmd/migrate` 经 `config.Load()` 读配置，`REDIS_URL`、`S3_*` 也是必填项，CI 步骤中为它们填了占位值（本步骤只使用 `DATABASE_URL`）。

## 迁移文件规范（`make migration-check`）
...
底层脚本为 POSIX sh；需要检查其他目录时可直接运行
`sh scripts/check-migrations.sh [可选目录]`。

## 部署配置门禁

`make deploy-check` 始终执行全部 Shell 语法与备份恢复回归测试，并校验四 target、non-root、
...
3. `contracts` 失败：先跑 `make contracts-check`；ProposalOperation 必须先改 `contracts/schemas/proposal-operation/v1/operation.schema.json`，再同步 `backend/internal/governance/schema/operation.schema.json`。
4. `migrations` 失败：先跑 `make migration-check` 看命名/成对/连续性；再对本地空库 `make migrate-up` 验证 SQL 本身。
5. `deploy` 失败：先跑 `make deploy-check`；迁移 gate/顺序失败不可跳过。OCI build 失败按具体 target 修复，禁止改成 root、可写根文件系统或移除 healthcheck 规避。
...
- 一次性全量本地自检：`make ci`（`make check` + 生成物漂移 + 安全扫描）。
