...
- 块编辑器只经 `components/editor/block-editor.tsx` 使用（ADR-0005）；BlockNote ↔ AST v1 双向映射在 `lib/editor/`，业务代码禁止直接引用 BlockNote API。
- 字体固定 Geist（layout 已挂载）；不引入第二套图标/字体/状态库。
- 提交前本地必须通过：`npm run typecheck && npm run lint && npm run build`。
