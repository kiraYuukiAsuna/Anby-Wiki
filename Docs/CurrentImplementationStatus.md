...
| 导入与 AI | URL/HTML/文本/PDF/PNG/JPEG/JSON/CSV 获取，图片及扫描 PDF 的中英 OCR、结构化数据规范化、分阶段进度、幂等 Job、解析成功后的不可变恢复点与无重复来源的失败重试、Semantic Kernel 结构化调用与纠正重试、精确引文的跨 Chunk 安全纠偏及坏候选隔离、带页码/坐标/置信度的证据约束抽取、Proposal 生成、可持久查询的导入队列 |
