...
通用关联。页面规划使用 `source-import-plan-v7`，多窗口结果由服务端
确定性合并，并由 `source-import-plan-fidelity-v5` 对照原始 Chunk 做证据化遗漏审计和章节级修复。
两者都把用户指定范围和块数上限视为跨窗口全局约束，并要求每条正文陈述由引文直接支撑；
导航标签、标题、孤立代码或示例值不能支撑宽泛说明。
