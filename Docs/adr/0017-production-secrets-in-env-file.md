...
- `DATABASE_URL`、`POSTGRES_PASSWORD`、`S3_ACCESS_KEY`、`S3_SECRET_KEY` 和
  `AUTH_DEV_LOGIN_TOKEN`，以及启用 AI 导入时的 `AI_API_KEY`，直接写入仓库外的
  `DEPLOY_ENV_FILE`。
...
- 环境文件泄露等同于数据库、对象存储和引导登录凭据同时泄露，必须使用 `0600`
  权限、最小化宿主机访问，并在泄露后整体轮换。
- 本决策只取代 ADR-0016 第 6 节，不改变共享引导令牌本身的早期阶段限制。
