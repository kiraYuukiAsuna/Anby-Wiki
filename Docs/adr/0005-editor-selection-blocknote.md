...
- M2-T02 以 Adapter 无损映射能力为验收门槛；若验证失败，按备选方案重估并更新本 ADR。
- 编辑器升级（BlockNote major version）视为契约变更，需人工核对 AST 往返语义并完成类型检查与构建。
...
- **已记录降级**：BlockNote 拒绝空 `initialContent`，空文档在编辑器内表现为一个空段落，
  序列化回 AST 会多出一个空 paragraph。BlockNote/ProseMirror 还会规范化相邻同样式
  文本节点并丢弃空文本节点，AST 侧若存在此类节点，经编辑器往返后会被合并/清除。
- 顶层孤立 list_item 摊平为 bulletListItem，
